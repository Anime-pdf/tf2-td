...
stock void Database_UpdatePlayer(int iUserId) {
	...
	g_hDatabase.Format(sQuery, sizeof(sQuery),
		...
		g_hSqlite ? "" : "LIMIT 1"); // actually im not sure it even makes sense, player_id is unique anyway
...
public void Database_OnUpdatePlayer_1(Handle hDriver, Handle hResult, const char[] sError, any iUserId) {
	if (hResult == null) {
		...
		g_hDatabase.Format(sQuery, sizeof(sQuery), 
			"INSERT %s IGNORE INTO `player_stats` (`player_id`, `map_id`, `first_connect`, `last_connect`, `last_disconnect`) " ...
			"VALUES (%d, " ...
					"(SELECT `map_id` " ...
					"FROM `server` " ...
					"WHERE `server_id` = %d " ...
					"LIMIT 1), " ...
					"%s)",
					g_hSqlite ? "OR" : "", iPlayerId, g_iServerId, g_hSqlite ? "datetime(\"now\"), datetime(\"now\"), datetime(\"now\"))" : "UTC_TIMESTAMP(), UTC_TIMESTAMP(), UTC_TIMESTAMP()");
...
stock void Database_CheckPlayerBanned(int iUserId) {
	...
	g_hDatabase.Format(sQuery, sizeof(sQuery),
		...
		"WHERE `player_id` = %d AND `expire` <= %s",
		iPlayerId, g_hSqlite ? "datetime(\"now\")" : "UTC_TIMESTAMP()");
	...
	if (hResult == null) {
		...
		g_hDatabase.Format(sQuery, sizeof(sQuery), 
			...
			"IN (SELECT MAX(`expire`) " ...
				...
				"WHERE `player_id` = %d) " ...
			"LIMIT 1",
			g_hSqlite ? "(`expire` || ' ' || 'UTC')" : "CONCAT(`expire`, ' ', 'UTC')",
			iPlayerId, iPlayerId);
...
public void Database_OnUpdatePlayerDisconnect_1(Handle hDriver, Handle hResult, const char[] sError, any iUserId) {
	if (hResult == null) {
		...
		g_hDatabase.Format(sQuery, sizeof(sQuery),
			...
			"WHERE `player_id` = %d " ...
			"%s",
			g_hSqlite ? "datetime(\"now\")" : "UTC_TIMESTAMP()",
			iPlayerId,
			g_hSqlite ? "" : "LIMIT 1");
...
public void Database_OnUpdatePlayerDisconnect_3(Handle hDriver, Handle hResult, const char[] sError, any iUserId) {
	if (hResult == null) {
		...
		g_hDatabase.Format(sQuery, sizeof(sQuery), 
			...
			"`metal_drop` = metal_drop + %d, `waves_played` = waves_played + %d, `wave_reached` =  %sIF(wave_reached < %d, wave_reached = %d, wave_reached), " ...
			"`rounds_played` = rounds_played + %d, `rounds_won` = rounds_won + %d, `playtime` = playtime + %d " ...
			"WHERE `player_id` = %d AND map_id = %d",
			iKills, iAssists, iDeaths, iDamage, iObjects_Built, iTowers_Bought, iMetal_Pick, iMetal_Drop, iWaves_Played, g_hSqlite ? "I" : "", iWaves_Reached, iWaves_Reached, iRounds_Played, iRounds_Won, iPlayTime, iPlayerId, g_iServerMap);
