...
stock void Database_LoadWaves() {
	...
	g_hDatabase.Format(sQuery, sizeof(sQuery), 
		"SELECT `wavetype`, `wave`.`name`, `class`, `quantity`, `health`, %sIF(`wavetype` & (SELECT `bit_value` FROM `wavetype` WHERE `wavetype`.`type` = 'air'), `teleport_air`, `teleport_ground`) " ...
		"FROM `wave` " ...
		"INNER JOIN `map` " ...
			"ON (`map`.`map_id` = %d) " ...
		"WHERE `wave_id` >= `wave_start` AND `wave_id` <= `wave_end` " ...
		"ORDER BY `wave_id` ASC",
		g_hSqlite ? "I" : "", g_iServerMap);
